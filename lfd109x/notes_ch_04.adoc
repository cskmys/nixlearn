...
----
+
This shows all the changes in `<file_directory>`, between two commits on the local repository.
Dropping `<file_directory>` will compare the whole project.
Dropping `<commit_id_2>`, will compare current branch with `<commit_id_1>`.
Dropping both `<commit_id_1>` and `<commit_id_2>`, will compare between the changes un-staged in the local copy and the last staged or committed changes on the current branch on the local repository.
