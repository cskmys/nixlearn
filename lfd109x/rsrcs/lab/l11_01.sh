...
lab(){
  ...
  my_echo "${PWD}"
  my_echo "cloning"
  #  git clone "git://127.0.0.1${TMP_DIR_PATH}/${remote_repo}" # if daemon launched as 'git daemon &'
  git clone "git://127.0.0.1/${remote_repo}" # if daemon launched as 'git daemon --base-path="${TMP_DIR_PATH}" &'

  ls -al
  ls -al "./${remote_repo}" # this is not the original remote repo,
                            # this is a repo inside the test repo which has the same name as remote repo
                            # as pwd is test repo, we are directly using the variable
  # test done
