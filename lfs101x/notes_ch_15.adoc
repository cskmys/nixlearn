...
This can lead to subtle problems, and it can be avoided by using the full path in the script such as `/bin/echo` to refer to external program instead of just `echo` which will refer to built-in command.
====
* Other scripts:
+
