...
lab(){
  echo -e "#!/bin/bash\necho \"Hello, this a phony ls program\"" > "${PWD}/ls"
  chmod +x "${PWD}/ls"
  echo "ls(original PATH)"
  ls -l "${PWD}"
  PATH_BCKP="${PATH}"
  export PATH="${PWD}:${PATH_BCKP}"
  echo "ls(after prepending pwd to original PATH)"
  ls -l "${PWD}"
  export PATH="${PATH_BCKP}:${PWD}"
  echo "ls(after appending pwd to original PATH)"
  ls -l "${PWD}"
  export PATH="${PATH_BCKP}"
  echo "ls(after restoring original PATH)"
  ls -l "${PWD}"
}
