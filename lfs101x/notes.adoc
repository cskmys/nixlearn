...
==== Logging in and out
On a local system, you can just enter the login username and password when prompted to login.
...
These commands sends a warning message and prevents further users from logging in.
...
Moreover, unlike hard links, soft links can point to objects even on different filesystems, partitions, and/or disks and other media, which may or may not be currently available or even exist.
