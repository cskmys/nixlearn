...
* Make sure cross-compiler toolchain is in your `PATH`
...
** run `make menuconfig` > navigate to `Load an Alternate Configuration File` > enter `../data/busybox-1.35.config` to load the configuration file provided at `./edt/embedded-linux-qemu-labs/tinysystem/data/busybox-1.35.config`

* Further, use `menuconfig` to configure more, as per your liking but make sure that:
** `Settings` >
...
+
You'll see that process `~bin/sh` restarting indefinitely.
If you press return, you'll see command prompt though.
However, you'll not find it easy or convenient to enter any commands.
+
All this is happening because `inittab` is not yet set.
