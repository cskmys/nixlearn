...
Now you cannot perform job control(run commands such as `halt` etc.), as `proc` filesystem is unavailable, and also you cannot read the system status as `sysfs` is unavailable.
Hence, you can do:

* At root create directories `proc`, `sys`
* Mount `proc` filesystem at `/proc`, and `sysfs` at `/sys`

Now, you have a decent system
